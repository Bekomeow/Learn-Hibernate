...
//        assertThat(results).hasSize(2);
//
//        List<String> names = results.stream().map(it -> it.get(0, User.class).fullName()).collect(toList());
//        assertThat(names).contains("Sergey Brin", "Steve Jobs");
//
//        List<Double> averagePayments = results.stream().map(it -> it.get(1, Double.class)).collect(toList());
